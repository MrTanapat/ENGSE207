...
**1. การใช้แนวคิด Separation of Concerns** <br>
   การแยก Logic ของการคำนวณเงิน ออกจาก Logic ของการเข้าถึงฐานข้อมูล เพื่อให้โค้ดสะอาดและดูแลรักษาง่ายขึ้น <br>
**2. การออกแบบโดยคำนึงถึงความล้มเหลว (Design for Failure)** <br>
   ให้ความสำคัญกับการจัดการ Exception และการทำ Error Handling ที่ดีขึ้น เพื่อไม่ให้ข้อผิดพลาดจุดเดียวทำให้โปรแกรมทั้งหมดหยุดทำงาน <br>
**3. การเลือกใช้เครื่องมือให้เหมาะสมกับงาน** <br>
   การเรียนรู้เรื่อง Database per Service ทำให้ผมเข้าใจว่าเราไม่จำเป็นต้องใช้ฐานข้อมูลประเภทเดียว (SQL) กับทุกงาน หากโปรเจกต์มีการเก็บข้อมูลตำแหน่ง
...
```
Architectural Style: คือ "ภาพรวม" หรือ "แนวทางหลัก" ในการจัดวางโครงสร้างระบบ เช่น Microservices, Layered, Client-Server
Architectural Pattern: คือ "วิธีแก้ปัญหาเฉพาะจุด" ที่เจอบ่อยๆ ภายใต้โครงสร้างนั้น เช่น MVC (Model-View-Controller) หรือ Singleton
สรุป: Style คือภาพใหญ่ของเมือง Pattern คือแบบแปลนของตึก
```
2. Monolithic Architecture เหมาะกับโปรเจกต์แบบไหน? ยกตัวอย่าง
```
Monolithic Architecture เหมาะกับโปรเจกต์: ไม่ซับซ้อนมาก, มีทีมพัฒนาขนาดเล็ก, ต้องการความเร็วในการทำตัวต้นแบบ (MVP) ออกมาให้ลูกค้ารันได้จริง
ยกตัวอย่าง: เว็บไซต์บล็อกส่วนตัว, ระบบจัดการสต็อกของร้านโชห่วยขนาดเล็ก, หรือแอปพลิเคชันที่เพิ่งเริ่มปั้นไอเดียใหม่ๆ ที่ยังไม่รู้ว่าจะมีคนใช้เยอะแค่ไหน
```
3. อธิบาย Separation of Concerns ใน Layered Architecture
```
คือการ "แบ่งหน้าที่ใครหน้าที่มัน" เช่น:
    - คนจัดหน้าจอ (UI Layer) ก็ทำแค่หน้าตา ไม่ต้องรู้เรื่องฐานข้อมูล
    - คนเขียนคำนวณ (Business Layer) ก็ทำแค่สูตรคำนวณ ไม่ต้องรู้ว่าหน้าจอสีอะไร
ข้อดี: เวลาอยากเปลี่ยนฐานข้อมูล เราก็แก้แค่ชั้นล่างสุด ไม่ต้องไปรื้อโค้ดส่วนที่วาดหน้าจอใหม่หมด
```
4. 2-Tier กับ 3-Tier Architecture แตกต่างกันอย่างไร?
```
2-Tier: คือ Client (แอป) คุยตรงกับ Database เลย 
3-Tier: มี "คนกลาง" (Application Server) มาคั่นระหว่างแอปกับ Database
ความต่าง: 3-Tier ปลอดภัยกว่าและจัดการ Logic ซับซ้อนได้ดีกว่า เพราะลูกค้าไม่เห็นหลังบ้านโดยตรง
```
5. Pipe-and-Filter Architecture เหมาะกับงานประเภทไหน?
```
เหมาะกับงาน: ประมวลผลข้อมูลเป็นลำดับขั้นตอนข้อมูลไหลเข้าช่องทางหนึ่ง (Pipe) ผ่านการคัดกรองหรือแปลงร่าง (Filter) แล้วส่งต่อไปยังขั้นตอนถัดไป
ยกตัวอย่าง: ระบบแปลงไฟล์วิดีโอ (แปลงไฟล์ -> ใส่ซับไตเติ้ล -> บีบอัด), ระบบวิเคราะห์ข้อมูล (Clean Data -> Transform -> Save)
```
6. ระบบ E-Commerce ควรเลือก Architecture แบบไหน? เพราะอะไร?
```
ควรเลือก Microservices (ร่วมกับ 3-Tier ในแต่ละ Service)
เพราะ: ระบบ E-Commerce มีหลายส่วนที่โหลดไม่เท่ากัน เช่น ช่วงเซลล์ 11.11 คนจะรุมดู "โปรโมชั่น" และ "จ่ายเงิน" มหาศาล
แต่คนจะไป "แก้โปรไฟล์" น้อยมาก การใช้ Microservices ทำให้เราขยาย (Scale) เฉพาะระบบจ่ายเงินให้รับโหลดไหวได้ โดยไม่ต้องเปลืองเงินไปขยายส่วนอื่น
```
7. Trade-offs สำคัญระหว่าง Monolithic กับ Layered คืออะไร?
```
Monolithic:

ข้อดี: พัฒนาเร็ว, Deploy ง่าย (ไฟล์เดียวจบ), ทดสอบง่าย

ข้อเสีย: พอนานไปโค้ดจะพันกันเหมือนสปาเกตตี แก้นิดเดียวพังทั้งแอป

Layered:

ข้อดี: โค้ดเป็นระเบียบ แยกกันชัดเจน ดูแลรักษาง่ายในระยะยาว

ข้อเสีย: การส่งข้อมูลข้ามชั้นอาจจะทำให้ช้าลงนิดหน่อย (Overhead) และเสียเวลาออกแบบตอนเริ่มมากกว่า
```
---
